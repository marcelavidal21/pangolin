/*** config start ***/
//var REMOTE_HOST = '23.22.144.165';  //Server 端 IP
/*** config end ***/
...
function connect(options){
  ...
  client.connect(options.remotePort, options.remoteHost, function() {
    ...
    client.on('data', function(data){
      ...
      if(/^port:/.test(data)){
        console.log('Connected to', options.remoteHost, options.remotePort);
        console.log('Proxy pass to', options.localHost, options.localPort);
        console.log('Please visit http://' + options.remoteHost + ':' + data.slice(5));
        //client.removeAllListeners('data');
        client.write('start server', 'utf-8');
        http2.raw.createServer(serverOpts, function(req, res) {
          var u = url.parse(req.url);
          req.headers.host = options.localHost + ':' + options.localPort;
